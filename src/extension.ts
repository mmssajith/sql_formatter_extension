...
function getConfiguration(): { dialect: string; rules: string[]; verboseLogging: boolean } {
...
function log(message: string, config: { verboseLogging: boolean }): void {
...
async function getSqlFluffPath(): Promise<string | null> {
	try {
...
async function formatSql(sql: string, config: { dialect: string; rules: string[]; verboseLogging: boolean }): Promise<string> {
	...
	if (!sqlfluffPath) {
		...
		const action = await vscode.window.showErrorMessage(
			'SQLFluff is not installed or not in PATH. Install it with "pip install sqlfluff".',
			'Install SQLFluff'
		);
